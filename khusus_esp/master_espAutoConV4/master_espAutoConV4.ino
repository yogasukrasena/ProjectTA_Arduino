...
void loop() {  
   if(Serial.available() && !onProcess){
    ...
    if(dataIn[0]=="espFirebase"){
      checkConn();
      if(modeESP8266>0){
        flagStatus(dataIn[4].toInt(),dataIn[1].toInt());
        dataRaw(dataIn[8].toInt(),dataIn[9].toInt(),dataIn[5].toInt(),dataIn[3]);
        dataLog(dataIn[10].toInt(),dataIn[11].toInt(),dataIn[12].toInt(),dataIn[13].toInt());        
        if(dataIn[1].toInt()>0){         
          dataGps(dataIn[6].toDouble(), dataIn[7].toDouble());         
        }
        if(Firebase.getString(fbdo,nameDevice+"/flag_status/status_finger")){
          ...
          if(dataFire.startsWith("enfinger")){          
            ...
            Serial.println(dataFire);                  
          }else if(dataFire.startsWith("done0")){
            Serial.println(dataFire);        
          }
        }
        if(dataIn[2].toInt() == 0){
          Firebase.setString(fbdo, nameDevice+"/flag_status/status_finger","noneAction");
        }else if(dataIn[2].toInt() == 1){
          Firebase.setString(fbdo, nameDevice+"/flag_status/status_finger","enDone");
        }else if(dataIn[2].toInt() == 2){
          Firebase.setString(fbdo, nameDevice+"/flag_status/status_finger","delDone");
        }else if(dataIn[2].toInt() == 3){
          Firebase.setString(fbdo, nameDevice+"/flag_status/status_finger","fail");
        }
      }
    }
