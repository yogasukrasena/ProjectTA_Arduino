...
void deviceData(int battery_value, int bpm_value, String fingger_data){
  char baterai_level[40], bpm_level[30], data_fingger[30];
  strcpy(baterai_level,nameDevice);
  strcpy(bpm_level,nameDevice);
  strcpy(data_fingger,nameDevice);
  if(Firebase.RTDB.setInt(&fbdo, strcat(baterai_level,"/raw_data/battery_level"), battery_value)&&
     Firebase.RTDB.setInt(&fbdo, strcat(bpm_level,"/raw_data/bpm_level"), bpm_value)){
...
//  deviceData(dt[2].toInt(),dt[3].toInt(),dt[5]); 
}
